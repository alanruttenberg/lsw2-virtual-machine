      ...
      (block nil
        (macrolet ((try (&body asdf-loading-forms)
          ...
          (let ((fasl (asdf-fasl-pathname)))
            (try (load fasl :verbose *load-verbose*))
            (try (load (compile-file source :verbose *compile-verbose* :output-file fasl))))
...
(let ((asdf-init (probe-file (qmerge "asdf-config/init.lisp"))))
  (when asdf-init
    (with-simple-restart (skip "Skip loading ~S" asdf-init)
      (load asdf-init :verbose *load-verbose* :print nil))))
...
;      (*load-verbose* nil)
      (*load-print* nil))
  (asdf:oos 'asdf:load-op "quicklisp" :verbose *load-verbose*))
